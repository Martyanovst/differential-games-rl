...
class Q_model(nn.Module):
    def __init__(self, p_model, phi_model,
    ...
    def forward(self, state, action):
        ...
        A = (-0.5) * p * \
            torch.bmm(action_phi.transpose(2, 1),
                      action_phi)[:, :, 0] + \
            p * torch.bmm(phi_delta.transpose(2, 1),
                          action_phi)[:, :, 0]
...
class Sphere_NAF:
    ...
    def __init__(self, p_model, phi_model, v_model,
        ...
        self.Q = Q_model(p_model, phi_model, v_model, action_shape, action_max)
